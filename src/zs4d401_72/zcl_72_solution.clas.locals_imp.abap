...
CLASS lcl_passenger_flight DEFINITION
  ...
  PROTECTED SECTION.
    METHODS
      get_description REDEFINITION. " RETURNING VALUE(r_result) TYPE string_table.
...
*    CLASS-DATA connections_buffer TYPE TABLE OF st_connections_buffer.
    CLASS-DATA connections_buffer TYPE HASHED TABLE OF st_connections_buffer
...
CLASS lcl_carrier DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    "! @raising zcx_72_failed | Instantiation failed - evaluate the exception text for details.
...
CLASS lcl_carrier IMPLEMENTATION.
  ...
  METHOD get_instance.
    ...
    SELECT SINGLE
     FROM /lrn/carrier
*    FIELDS concat_with_space( carrier_id, name, 1 ),
    FIELDS concat_with_space( carrier_id, name, 1 ) AS name,
           currency_code
    WHERE carrier_id = @i_carrier_id
*    INTO ( @DATA(name), @me->currency_code ).
    INTO @DATA(details).
...
*          value = CONV #( i_carrier_id ).
      ...
      RAISE EXCEPTION TYPE zcx_72_failed
        EXPORTING
          textid     = zcx_72_failed=>carrier_not_found
          carrier_id = i_carrier_id.
    ENDIF.
...
*          textid = cx_abap_auth_check_exception=>missing_authorization.
      RAISE EXCEPTION TYPE zcx_72_failed
        EXPORTING
          textid     = zcx_72_failed=>carrier_no_read_auth
          carrier_id = i_carrier_id.
    ENDIF.
